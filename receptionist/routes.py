...
async def websocket_endpoint(
    websocket: WebSocket, service: ReceptionistService = Depends(get_receptionist_service)
):
    """
    ...
    try:
        ...
        try:
            while True:
                ...
                response_message: str = await service.handle_message(message, message_history)
                audio_data = await service.generate_audio(response_message)
                response = {
                    "type": "bot",
            ...
            logger.error(
                "Error in websocket communication (client: %s): %s", client_id, e, exc_info=True
            )
            try:
        ...
        logger.error(
            "Error accepting websocket connection (client: %s): %s", client_id, e, exc_info=True
        )
